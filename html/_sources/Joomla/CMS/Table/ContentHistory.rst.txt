----------------------------------
Joomla\\CMS\\Table\\ContentHistory
----------------------------------
...
.. php:class:: ContentHistory
    ...
    .. php:method:: getSha1($jsonData, ContentType $typeTable)

        Utility method to get the hash after removing selected values. This lets
        us detect changes other than
        modified date (which will change on every save).

        :param $jsonData:
        :type $typeTable: ContentType
        :param $typeTable:
        :returns: string SHA1 hash on success. Empty string on failure.

    .. php:method:: getHashMatch()
    ...
    .. php:method:: deleteOldVersions($maxVersions)

        Utility method to remove the oldest versions of an item, saving only the
